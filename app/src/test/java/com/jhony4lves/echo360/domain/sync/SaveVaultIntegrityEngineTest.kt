...
class SaveVaultIntegrityEngineTest {
    ...
    fun `missing size and hash mismatches are explicit errors`() {
        val report = SaveVaultIntegrityEngine.verify(manifest, listOf(evidence("a.bin", 2L, "c")))
    ...
    fun `extra local payload is warning and does not need a hash`() {
        val report = SaveVaultIntegrityEngine.verify(
            manifest = manifest,
            localFiles = listOf(evidence("a.bin", 3L, "a"), evidence("sub/b.bin", 4L, "b")),
    ...
    @Test
    fun `wrong object type is distinct from missing`() {
        val report = SaveVaultIntegrityEngine.verify(
            manifest = manifest,
            localFiles = listOf(evidence("a.bin", 3L, "a")),
            wrongObjectTypePaths = listOf("sub/b.bin"),
        )
        assertFalse(report.valid)
        assertFalse(report.complete)
        assertTrue(report.findings.any { it.code == SaveVaultIntegrityCode.WrongObjectType })
        assertFalse(report.findings.any { it.code == SaveVaultIntegrityCode.MissingFile && it.relativePath == "sub/b.bin" })
    }
    ...
    fun `path matching is case insensitive like manifest duplicate policy`() {
        val report = SaveVaultIntegrityEngine.verify(
            manifest,
            listOf(evidence("A.BIN", 3L, "a"), evidence("SUB/B.BIN", 4L, "b")),
        )
        assertTrue(report.valid)
        assertTrue(report.complete)
    }
