...
class FtpRangeReadIntegrationTest {
    @Test
    fun `Aurora bounded REST read returns only requested passive range`() {
        val payload = ByteArray(256 * 1024) { index -> (index * 19).toByte() }
        val offset = 73_211L
        val count = 4_321
        val server = RangeFakeFtpServer.start(Mode.Passive, payload)
        ...
        val bytes = runBlocking {
            ...
            try {
                session.readRangeAndClose("/Hdd1/Games/Data0003", offset, count)
            } finally {
        ...
        server.await()
        assertArrayEquals(payload.copyOfRange(offset.toInt(), offset.toInt() + count), bytes)
        assertTrue(server.commands.any { it == "REST $offset" })
        assertTrue(server.commands.any { it == "RETR /Hdd1/Games/Data0003" })
    }

    @Test
    fun `FTPdll bounded REST read returns only requested active range`() {
        val payload = ByteArray(256 * 1024) { index -> (255 - index).toByte() }
        val offset = 91_007L
        val count = 7_777
        val server = RangeFakeFtpServer.start(Mode.Active, payload)
        ...
        val bytes = runBlocking {
            ...
            try {
                session.readRangeAndClose("/Hdd1/Content/Data0007", offset, count)
            } finally {
                runCatching { session.close() }
            }
        }
    ...
    private class RangeFakeFtpServer private constructor(
        ...
        fun await() {
            ...
            if (worker.isAlive) {
                runCatching { controlListener.close() }
                fail("Range fake FTP server did not terminate. commands=$commands")
            }
            failure.get()?.let { throw AssertionError("Range fake FTP failed. commands=$commands", it) }
        }
        ...
        private fun serve() {
            controlListener.accept().use { control ->
                ...
                reply("220 Echo360 range fake ready")
                while (true) {
                        ...
                        "RETR" -> {
                            ...
                            withDataSocket { data ->
                                try {
                                    ...
                                    var position = restartOffset
                                    while (position < payload.size) {
                                        val count = minOf(1024, payload.size - position)
                                        output.write(payload, position, count)
                                        output.flush()
                                        position += count
                                    }
                                } catch (_: IOException) {
                                    // Expected for bounded reads: Echo360 closes
                                    // the data socket after collecting its range.
                                }
                            }
                            runCatching { reply("226 Transfer complete") }
                            passiveListener?.close()
                            return
                        }
                        "QUIT" -> {
                            runCatching { reply("221 Goodbye") }
                            passiveListener?.close()
                            return
                        }
        ...
        companion object {
            fun start(mode: Mode, payload: ByteArray): RangeFakeFtpServer = RangeFakeFtpServer(
