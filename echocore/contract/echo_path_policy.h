...
/*
 * Read-only path gate for v1. Accepts the Hdd1 root as either "Hdd1:" or
 * "Hdd1:/", plus descendants using either slash style.
 *
...
static inline int echo_path_is_safe_readonly(const char *path, size_t length) {
    ...
    if (path == NULL || length < 5U || length > ECHO_PATH_MAX) return 0;
    if (!echo_path_has_allowed_root(path, length)) return 0;

    /* The bare canonical mount name is a valid stat target. */
    if (length == 5U) return 1;

    if (path[5] != '/' && path[5] != '\\') return 0;

    /* A single separator after the root is also the mount root. */
 ...
 * Root spellings normalize to "\\Device\\Harddisk0\\Partition1" without a
 * trailing separator. Output is NUL-terminated for RtlInitAnsiString /
 * OBJECT_ATTRIBUTES use.
 */
static inline int echo_path_to_native_hdd1(
    ...
    suffix_length = length <= 6U ? 0U : length - 5U;
    if (prefix_len + suffix_length + 1U > out_capacity) return -2;
