...
/* Prevent freestanding aggregate-zeroing from becoming an implicit memset. */
static void echo_doctor_zero(void *memory, size_t length) {
...
void echo_xbox_read_memory(echo_doctor_memory *memory) {
    MM_QUERY_STATISTICS_RESULT stats;
    NTSTATUS status;
    ...
    echo_doctor_zero(&stats, sizeof(stats));
...
void echo_xbox_read_temperature(echo_doctor_temperature *temperature) {
    uint8_t message[ECHO_SMC_MESSAGE_BYTES];
    uint8_t response[ECHO_SMC_MESSAGE_BYTES];
    uint16_t cpu;
    uint16_t gpu;
    uint16_t memory;
    uint16_t box;
    ...
    echo_doctor_zero(message, sizeof(message));
    echo_doctor_zero(response, sizeof(response));
