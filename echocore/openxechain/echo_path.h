...
#define ECHO_CANONICAL_PATH_MAX_BYTES 240U
...
static inline int echo_validate_canonical_path(const char *path, size_t length) {
    ...
    if (length > ECHO_CANONICAL_PATH_MAX_BYTES) return -6;
...
static inline echo_path_info echo_classify_canonical_path(const char *path, size_t length) {
    ...
    if (echo_segment_equals(path + 1U, end - 1U, "Hdd1")) {
        result.device = ECHO_DEVICE_HDD1; result.writable = 1;
    } else if (echo_segment_equals(path + 1U, end - 1U, "Usb0")) {
        result.device = ECHO_DEVICE_USB0; result.writable = 1;
    } else if (echo_segment_equals(path + 1U, end - 1U, "Flash")) {
        result.device = ECHO_DEVICE_FLASH; result.writable = 0;
    }
    return result;
}
...
static inline const char *echo_kernel_prefix(echo_device_kind device) {
    switch (device) {
        case ECHO_DEVICE_HDD1: return "\\Device\\Harddisk0\\Partition1";
        case ECHO_DEVICE_USB0: return "\\Device\\Mass0";
        case ECHO_DEVICE_FLASH: return "\\SystemRoot";
        default: return NULL;
    }
}
...
static inline size_t echo_path_to_kernel(const char *canonical, size_t canonical_length,
    char *output, size_t output_capacity, int require_write) {
    echo_path_info info = echo_classify_canonical_path(canonical, canonical_length);
    const char *prefix;
    size_t prefix_length, segment_end, suffix_length, total, i;
